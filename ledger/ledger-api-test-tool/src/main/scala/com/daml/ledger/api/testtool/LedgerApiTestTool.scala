...
object LedgerApiTestTool {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val missingTests = (config.included ++ config.excluded).filterNot(pref =>
      Tests.all(config).map(_.name).exists(test => test.startsWith(pref)))
    ...
    val shouldRun: String => Boolean = name => {
      included_p.filter(name.startsWith(_)).nonEmpty && config.excluded
